...
# Parameters: que (list) - the queue of orders
#             a (int) - the food item chosen by the customer
# Returns: None
def enque(que, a):
    que.append(a)
...
# File: [File Name]
# Overview:
# This file contains code for a restaurant menu system. It allows the user to select a dish from the menu, enter the quantity, and calculate the total price. The selected dishes are added to a queue and can be dequeued or displayed.

# Functions/Methods:

# enque(que, a)
# Description: Adds an element to the queue.
# Parameters: que (queue) - The queue to add the element to
#             a (int) - The element to add to the queue
# Returns: None

# deque(que)
# Description: Removes an element from the queue.
# Parameters: que (queue) - The queue to remove the element from
# Returns: None

# display(que)
# Description: Displays the elements in the queue.
# Parameters: que (queue) - The queue to display
# Returns: None
```
...
// Parameters: None
// Returns: 0 if the program executed successfully
int main() {
    while ('y' == getch()); // Wait for user input
    display(que); // Call the display function to show the queue
    printf("\n\n\t=================FOS=================          "); // Print a heading
